...
- `nsg_configs` is a map keyed by a unique logical name (e.g. `nsg_alpha`, `nsg_beta`). Use `create_nsg = true` to create an NSG, or `create_nsg = false` with `existing_nsg_name` / `existing_nsg_id` to reference an existing NSG.
